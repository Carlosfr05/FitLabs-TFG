...
Para el módulo de búsqueda y selección de ejercicios al crear rutinas, FitLabs evaluó inicialmente una API REST externa que proporcionaba un catálogo extenso de ejercicios con información detallada (nombre, grupo muscular, equipamiento necesario, imagen demostrativa).

La integración se llegó a prototipar mediante el paquete \texttt{http} de Flutter, con peticiones GET autenticadas mediante \textit{API Key} en los headers de la solicitud. Sin embargo, la API pasó a un modelo de pago y la solución final se recondujo a un catálogo local para garantizar estabilidad y continuidad del proyecto.
...
La identidad visual de FitLabs se basa en una paleta en tonos púrpura oscuro (\texttt{\#3D2B6B} como color primario), que transmite profesionalidad y dinamismo. El prototipo puede consultarse en \url{https://www.figma.com/design/xCVkUSQO1mjtpJQ3Z64uXH/Sin-título?node-id=0-1&t=Yd1SPcBnxRwi8qKQ-1}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{fitlabs_figma.png}
  \caption{Prototipo de diseño en Figma con pantallas principales de FitLabs.}
  \label{fig:figma_prototipo}
\end{figure}
\FloatBarrier

% Se ha eliminado el gráfico comparativo de frameworks por indicación del autor.
...
\begin{itemize}
  \item Utilizar inicialmente una API externa para el catálogo de ejercicios, con un enfoque centrado en el consumo de datos de terceros.
  \item Mantener el alcance en las funcionalidades principales del entrenador, priorizando autenticación, gestión de clientes y creación de rutinas.
  \item Desarrollar una interfaz visual propia, pero sin cerrar todavía un sistema de diseño tan definido como el adoptado finalmente.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Catálogo de ejercicios local}: La sustitución de la API externa por un catálogo local fue una decisión de estabilidad y cierre de proyecto, permitiendo mantener la funcionalidad sin depender de cambios comerciales o límites del proveedor.
\end{enumerate}
...
API de ejercicios & No contemplada & Catálogo local de ejercicios \\
\bottomrule
