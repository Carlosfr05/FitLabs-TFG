...
\section{Evaluación crítica del resultado}

Aunque el resultado final es satisfactorio y cumple los objetivos funcionales definidos, se identifican aspectos que hubieran elevado aún más la calidad técnica si el calendario hubiese sido más amplio:

\begin{itemize}
  \item \textbf{Automatización de pruebas}: la validación principal ha sido manual y semiautomática. Una suite de tests más extensa reduciría riesgo de regresión.
  \item \textbf{Observabilidad}: faltan métricas de telemetría en producción (tiempos de respuesta por módulo, errores por pantalla, ratio de abandono).
  \item \textbf{Arquitectura de estado}: el enfoque actual funciona para MVP, pero una solución tipo BLoC/Provider mejoraría escalabilidad.
\end{itemize}

Lejos de considerarse una carencia grave, estas limitaciones son propias de un proyecto académico con alcance acotado y se convierten en oportunidades claras de evolución.

\section{Aportación del proyecto al perfil profesional DAM}

El desarrollo de FitLabs ha permitido consolidar competencias nucleares del perfil de Técnico Superior en Desarrollo de Aplicaciones Multiplataforma:

\begin{enumerate}
  \item \textbf{Diseño y desarrollo móvil}: construcción de una app completa en Flutter con navegación, formularios y consumo de servicios.
  \item \textbf{Persistencia y seguridad}: modelado relacional y aplicación de políticas RLS en Supabase.
  \item \textbf{Trabajo colaborativo}: uso de Git en ramas, integración progresiva y resolución de conflictos.
  \item \textbf{Documentación técnica}: capacidad para justificar decisiones de análisis, diseño, pruebas y despliegue.
\end{enumerate}

En términos de empleabilidad, este proyecto constituye una evidencia práctica de autonomía técnica, criterio de diseño y capacidad de entrega integral.

\section{Cierre final}

FitLabs cumple el objetivo principal del TFG: demostrar, mediante un caso realista y funcional, la adquisición de competencias técnicas y metodológicas del ciclo DAM. El producto final no es únicamente una maqueta, sino una base sólida sobre la que construir una solución profesional de mayor alcance.
