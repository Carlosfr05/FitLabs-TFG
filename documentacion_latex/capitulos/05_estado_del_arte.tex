...
La tendencia más relevante para FitLabs no es solo el crecimiento del mercado, sino la especialización: las soluciones que mejor encajan son las que resuelven un flujo de trabajo concreto para entrenadores, en lugar de intentar cubrir todo el ecosistema del fitness con una plataforma genérica.
...
Para el módulo de creación de rutinas, FitLabs evaluó una API externa de ejercicios durante la fase de desarrollo. Las principales opciones consideradas fueron:

\begin{itemize}
  \item \textbf{ExerciseDB} (RapidAPI): Amplio catálogo con imágenes GIF animadas de demostración. Fue la opción más cercana al enfoque inicial del proyecto, pero terminó descartándose por pasar el servicio a un modelo de pago y por la necesidad de depender de cuotas externas.
...
Durante el desarrollo se llegó a implementar una primera versión de integración HTTP con ExerciseDB. El historial del repositorio conserva esa aproximación, incluyendo la llamada REST con paginación y timeout. Un fragmento representativo de esa implementación fue el siguiente:

\begin{lstlisting}[caption={Fragmento histórico de la integración con ExerciseDB}]
...
La versión final del proyecto sustituye esa dependencia por un catálogo local de ejercicios, que ofrece mayor estabilidad y evita bloqueos por cambios en las condiciones del proveedor.
